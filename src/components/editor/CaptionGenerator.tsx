...
import { useEditorStore, Caption } from '../../lib/store';

export default function CaptionGenerator() {
  const { currentProject, addCaptions, setIsProcessing } = useEditorStore();
  ...
  const generateCaptions = async () => {
    ...
    try {
      const response = await fetch('https://api.example.com/generate-captions', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="flex flex-col items-center justify-center p-4">
      ...
      >
        {isGenerating ? <Loader className="animate-spin mr-2" /> : <Type className="mr-2" />}
